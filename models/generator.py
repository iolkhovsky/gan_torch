...
class FaceGenerator(nn.Module):
    ...
    def __init__(self, upsample_type="TransposeConv"):
        super(FaceGenerator, self).__init__()
        self.fc1 = nn.Linear(64, 4*4*1024, bias=False)
        self.act = nn.LeakyReLU(0.2)
        self.bn = nn.BatchNorm1d(4*4*1024)
        self.up1 = UpSample([1024, 512], upsample_type)
        self.up2 = UpSample([512, 256], upsample_type)
        self.up3 = UpSample([256, 128], upsample_type)
        self.up4 = UpSample([128, 3], upsample_type)
        self.act = nn.Tanh()
    ...
    def forward(self, x):
        ...
        x = self.act(x)
        x = x.view(-1, 1024, 4, 4)       # 4*4*512 -> 512x4x4
...
class TestGenerator(unittest.TestCase):
    ...
    def test_forward_transpose_conv(self):
        batch_sz = 2
        model = FaceGenerator("TransposeConv")
